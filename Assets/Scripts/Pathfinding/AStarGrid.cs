...
public class AStarGrid : MonoBehaviour
{
    ...
    public void CreateGrid()
    {
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            for (int y = 0; y < gridSizeY; y++)
            {
                ...
                if (Physics2D.OverlapCircle(worldPoint, overlapCircleRadius, unwalkableMask) != null)
                {
                    walkable = Node.Walkable.Obstacle;
                }
                else if (Physics2D.OverlapCircle(worldPoint, overlapCircleRadius, slowMask) != null)
                {
                    walkable = Node.Walkable.Slow;
                }
                else
                {
                    walkable = Node.Walkable.Walkable;
                }
    ...
    public List<Node> GetNeighbours(Node node)
    {
        ...
        if (includeDiagonalNeighbours) {
            ...
            // Top-Right
            if(node.gridY - 1 > 0 && node.gridX + 1 < gridSizeX) {
                neighbours.Add(grid[node.gridX + 1, node.gridY - 1]);
            }

            // Bottom-Left
            if(node.gridY + 1 < gridSizeY && node.gridX - 1 > 0) {
                neighbours.Add(grid[node.gridX - 1, node.gridY + 1]);
            }

            // Bottom-Right
            if(node.gridY + 1 < gridSizeY && node.gridX + 1 < gridSizeX) {
                neighbours.Add(grid[node.gridX + 1, node.gridY + 1]);
            }
        }
    ...
    Node FindWalkableInRadius(int centreX, int centreY, int radius)
    {
        for (int i = -radius; i <= radius; i++)
        {
            ...
            {
                if (grid[verticalSearchX, centreY + radius].walkable != Node.Walkable.Obstacle)
                {
            ...
            {
                if (grid[centreX + radius, horizontalSearchY].walkable != Node.Walkable.Obstacle)
                {
    ...
    void OnDrawGizmos()
    {
        ...
        if (grid != null && displayGridGizmos)
        {
            ...
            {
                if (n.walkable == Node.Walkable.Obstacle)
                {
                    Gizmos.color = Color.red;
                }
                else if (n.walkable == Node.Walkable.Slow)
                {
